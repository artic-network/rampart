...
const drawMaxLines = (svg, scales, data, colour) => {
    ...
    svg.selectAll(".maxCoverageLine")
        ...
        .attr("stroke-width", 1)
        .attr('d', (coverageKey) => {
            const coverage = data[data.length - 1][coverageKey];
            const firstTime = data[0].time;
            const lastTime = data[data.length - 1].time;
            return `M ${scales.x(firstTime)},${scales.y(coverage)} L ${scales.x(lastTime)},${scales.y(coverage)}`;
        });
    ...
    svg.selectAll(".maxCoverageText")
        ...
        .attr("x", (coverageKey, index) => {
            const firstTime = scales.x(data[0].time);
            const lastTime = scales.x(data[data.length - 1].time);
            return (index === 0 ? firstTime : (index === 2 ? lastTime : (lastTime - firstTime) / 2));
        })
        .attr("y", (coverageKey) => {
            const coverage = data[data.length - 1][coverageKey];
            return scales.y(coverage);
        })
        .attr("dy", "12px") /* positive values bump down text */
        .attr("text-anchor", (coverageKey, index) => {
            return (index === 0 ? "start" : (index === 2 ? "end" : "center"));

        })
        .attr("baseline-shift", "120%") /* i.e. y value specifies top of text */
        .attr("pointer-events", "none") /* don't capture mouse over */
        .text((coverageKey, index) => {
            const coverage = data[data.length - 1][coverageKey];
            return `>${labels[index]} = ${coverage}%`;
        });
};
